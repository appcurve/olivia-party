...
DOCKER_VERSION_TAG="v0.0.0-alpha.7"
...
# if there are issues w/ cache you may need to `docker compose build --no-cache --pull [SERVICE_NAME]``
...
# push to ecr with both version and 'latest' tags
# the image data will only be uploaded the first time; the two push commands set both tags
